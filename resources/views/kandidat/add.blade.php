...
@section('title', 'Add Kandidat Tahun ' . $periode->nama_periode)
...
@section('content')
    <div class="row">
        <div class="col-md-6">
            <h1 class="h3 mb-4 text-gray-800">Add Kandidat Tahun {{ $periode->nama_periode }}</h1>
        </div>
        <div class="col-md-6">
            <ol class="breadcrumb float-sm-right" style="background-color: none;">
                <li class="breadcrumb-item active"><small>Add Kandidat Tahun {{ $periode->nama_periode }}</small></li>
            </ol>
        </div>
    </div>
        ...
        <div class="col-md">
            <div class="card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Add Kandidat Tahun {{ $periode->nama_periode }}</h6>
                </div>
                <div class="card-body">
                    <form action="{{ url('kandidat') }}" class="form" method="post">
                        @csrf
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('nama_kandidat') is-invalid @enderror"
                                name="nama_kandidat" id="nama_kandidat" value="{{ old('nama_kandidat') }}">
                            @error('nama_kandidat')
